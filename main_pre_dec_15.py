...
W_c_hat =np.asarray(yay[3:18])
W_a_hat = np.asarray(yay[18:24]).reshape((n,m))
...
vals_lqr=integrate.odeint(sys_func, x_init,  t_eval, args=((K_lqr,)))
...
# W_a_opt=np.array(-K) 
# W_a_opt = np.atleast_2d(W_a_opt).T
...
while t_span_ac[1]<=t_span[1]:
    if t_span_ac[1] >= 3*int(t_span[1]/5):
        explore=0
...
vals_lqr_new=integrate.odeint(sys_func, x_init,  t_eval, args=((-W_a_hat.T,)))
